...
from app.models.tickets import (
    Ticket, TicketInitiator, TicketEvent, TicketStatusDefinition
)
...
def generate_ticket_number(db: Session) -> str:
    ...
    latest = db.query(Ticket).order_by(Ticket.created_at.desc()).first()

    if not latest:
        return "T-000001"

    # Extract number from latest ticket
    try:
        num = int(latest.ticket_number.split('-')[1])
        return f"T-{num + 1:06d}"
    except:
        return "T-000001"
...
async def list_tickets(
    ...
    status_id: Optional[UUID] = Query(None, description="Filter by status"),
...
async def create_ticket(
    ...
    **Validation:**
    ...
    - Either contact_person_id or contact_name is required
    ...
    if ticket_data.contact_person_id:
    ...
    # Validate: either contact_person_id or contact_name must be provided
    if not ticket_data.contact_person_id and not ticket_data.contact_name:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Either contact_person_id or contact_name must be provided"
        )
    ...
    # Create ticket initiator
    actor_type, actor_id, actor_display = get_actor_info(current_user)

    # Use provided initiator info or default to current user
